...
from parameterizedAlgorithm import paramAlgorithm, applyReductions
import sys
...
def reduction2(
    G, k
):  # Se G possui um vértice com grau de pelo menos k + 1, então a nova instância é (G - v, k - 1).
    for v in G.V():
        d = grau(G, v)
        if d >= (k + 1):
...
def vertexCover(G, k, stop):
    ...
    for cover_candidate in itertools.combinations(G.V(), k):
        ...
        cover_candidate_ad = [False] * (G.n + 1)
        for v in cover_candidate:
            cover_candidate_ad[v] = True
        ...
        for u in G.V():
            ...
            while current_node:
                ...
                if cover_candidate_ad[u] == False and cover_candidate_ad[v] == False:
